...
from utils import QueryCounterCursor
...
if __name__ == "__main__":
    ...
    # cursor = conn.cursor()
    cursor = QueryCounterCursor(conn.cursor())
    ...
    try:
        ...
        while True:
            try:
                ...
                print(f"Total DB Queries this loop: {cursor.query_count}")
                if cursor.query_count > 0:
                    report_path = "query_report.log"
                    with open(report_path, "a", encoding="utf-8") as f:
                        f.write(f"\n=== Loop at {current_timestamp} ({cursor.query_count} queries) ===\n")
                        for entry in cursor.query_log:
                            f.write(f"[{entry['location']}] → {entry['query']}\n")
                cursor.query_count = 0  # Reset query count for the next loop
                cursor.query_log = []  # Reset query log for the next loop
